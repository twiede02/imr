...
struct SPIRVModule {
    ...
    SPIRVModule(const std::string& filename) noexcept(false) : name(filename) {
...
struct ReflectedLayout {
    ...
    ReflectedLayout(SPIRVModule& spirv_module, VkShaderStageFlags stage) : stages(stage) {
    ...
    ReflectedLayout(ReflectedLayout& a, ReflectedLayout& b) noexcept(false) : push_constants(a.push_constants), set_bindings(a.set_bindings), stages(a.stages | b.stages) {
...
struct PipelineLayout {
    ...
    PipelineLayout(imr::Device& device, ReflectedLayout& reflected_layout) : device(device) {
        int max_set = 0;
...
struct ShaderModule {
    ...
    ShaderModule(imr::Device& device, SPIRVModule& spirv_module) noexcept(false) : device(device) {
...
struct ComputeShader::Impl {
    ...
    Impl(imr::Device& device, std::string&& name, std::string&& entrypoint_name) : device(device) {
        auto shader_stage = VK_SHADER_STAGE_COMPUTE_BIT;
        auto spirv_module = SPIRVModule(name);
        auto reflected = ReflectedLayout(spirv_module, shader_stage);

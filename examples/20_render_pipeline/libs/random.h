...
inline unsigned int FNVHash(char* str, unsigned int length) {
...
// FNV hash function
inline auto fnv_hash(uint32_t h, uint32_t d) -> uint32_t {
...
inline unsigned int nrand(unsigned int* rng) {
    unsigned int orand = *rng;
    *rng = FNVHash((char*) &orand, 4);
    return *rng;
}

inline auto xorshift(uint32_t* seed) -> uint32_t {
    auto x = *seed;
    // x = select(x == 0u, 1u, x);
    x = (x == 0) ? 1u : x;
    x ^= x << 13u;
...
// [0.0, 1.0]
inline auto randf(RNGState* rnd) -> float {
...
inline auto randi_max(RNGState* rnd, uint32_t max) -> uint32_t {
